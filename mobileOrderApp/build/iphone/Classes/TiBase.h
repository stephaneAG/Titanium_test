...
// in simulator we redefine to format for mobileOrderApp Developer console
...
{\
	NSString * exceptionName = [@"org.mobileorderapp." stringByAppendingString:NSStringFromClass([self class])];\
...
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.mobileorderapp";\
	NSString * message = [NSString stringWithFormat:@"%@. %@ %@",reason,(subreason!=nil?subreason:@""),(location!=nil?location:@"")];\
	NSLog(@"[ERROR] %@",message);\
	if ([NSThread isMainThread]==NO) {\
		@throw [NSException exceptionWithName:exceptionName reason:message userInfo:nil];\
...
enum {
	UImobileOrderAppNativeItemNone = -1, 
	UImobileOrderAppNativeItemSpinner = -2,
	UImobileOrderAppNativeItemProgressBar = -3,
	
	UImobileOrderAppNativeItemSlider = -4,
	UImobileOrderAppNativeItemSwitch = -5,
	UImobileOrderAppNativeItemMultiButton = -6,
	UImobileOrderAppNativeItemSegmented = -7,
	
	UImobileOrderAppNativeItemTextView = -8,
	UImobileOrderAppNativeItemTextField = -9,
	UImobileOrderAppNativeItemSearchBar = -10,
	
	UImobileOrderAppNativeItemPicker = -11,
	UImobileOrderAppNativeItemDatePicker = -12,
	
	UImobileOrderAppNativeItemInfoLight = -13,
	UImobileOrderAppNativeItemInfoDark = -14,
	
	UImobileOrderAppNativeItemDisclosure = -15,
	
	UImobileOrderAppNativeItemContactAdd = -16
};
