...
@implementation mobileOrderAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mobileorderapp);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any mobileOrderApp patches in JS before a context is loaded... nice for 
...
{
	// create mobileOrderApp global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mobileorderapp = [[mobileOrderAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mobileorderapp];
	
	NSString *_mobileorderappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mobileorderappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mobileorderapp valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.mobileorderapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
