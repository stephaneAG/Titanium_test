...
@implementation gesturesDebugAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_gesturesdebugapp);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any gesturesDebugApp patches in JS before a context is loaded... nice for 
...
{
	// create gesturesDebugApp global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_gesturesdebugapp = [[gesturesDebugAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_gesturesdebugapp];
	
	NSString *_gesturesdebugappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _gesturesdebugappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_gesturesdebugapp valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.gesturesdebugapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
