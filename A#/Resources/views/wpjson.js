...
//la function qui charge les differentes geolocs
function loadGeolocs(){
	//array vide pour notre table View
	var rowData = [];
	...
	//on spécfifie la requete http et l url des datas
	//loader.open( "GET" , "http://stephaneadamgarnier.com/porpoise/config/GeolocsForRowsJSON.php?fakeJSON" );
	//loader.open( "GET" , "http://stephaneadamgarnier.com/porpoise/config/geolocalisations.json" );
	loader.open( "GET" , "http://stephaneadamgarnier.com/porpoise/config/fakeJSON.json" );
	//on lance notre function quand les data sont pretes pr le process
	loader.onload = function(){
		//on evalue le JSON
		//var geolocs = eval( '(' + this.responseText + ')' );
		var geolocs = JSON.parse(this.responseText);
		
		for ( var i=0; i < geolocs.length; i++ ){
			
			
			var latitude = geolocs[i].anchor.geolocation.lat; //la latitude
			var longitude = geolocs[i].anchor.geolocation.lon; //la longitude
			var altitude = geolocs[i].anchor.geolocation.alt; //l altitude
			
			var id = geolocs[i].id; // l id
			var imageUrl = geolocs[i].imageURL; // l'url de l'icone associée
			var description = geolocs[i].text.description; //la description associée
			var title = geolocs[i].text.title; // le titre
			
			var tweet = geolocs[i].text; //le message du tweet
			var user = geolocs[i].user.screen_name; //le nom du user
			var avatar = geolocs[i].user.profile_image_url; //l'image du profil
			...
			var av_image = Titanium.UI.createImageView({
				//url: avatar, //l'image de notre imageView
				image: imageUrl, //l'image de notre imageView
				top:0 ,
				left: 0 ,
				height: 48 ,
				width: 48
